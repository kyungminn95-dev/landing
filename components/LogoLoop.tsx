...
const useResizeObserver = (callback: () => void, elements: React.RefObject<Element | null>[], dependencies: unknown[]) => {
  useEffect(() => {
...
export const LogoLoop = memo(({
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: React.Key) => {
    ...
    const content = isNodeItem
      ...
      : <img src={(item as ImageLogoItem).src} srcSet={(item as ImageLogoItem).srcSet} alt={(item as ImageLogoItem).alt ?? ""} title={item.title} loading="lazy" decoding="async" draggable={false} />;
